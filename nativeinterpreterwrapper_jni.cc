...
// extern BufferErrorReporter* buffer = new BufferErrorReporter;
/**
namespace {
...
//   // buffer->outInfo();
//   BufferErrorReporter* buffer = new BufferErrorReporter(errorInfo);
...
/** LiYu*/
tflite::FlatBufferModel* createModelWithBuffer(
    /**JNIEnv* env, jclass /*clazz/, jobject model_buffer, jlong error_handle*/) {
  // BufferErrorReporter* error_reporter =
  //     convertLongToErrorReporter(env, error_handle);
  // if (error_reporter == nullptr) return 0;
  ...
  cout << "createModelWithBuffer " << endl;
  return reinterpret_cast<tflite::FlatBufferModel*>(model);
}
